...
class TasksTest extends AbstractFunctional
{
    ...
    {
        $this->sdk->Profile($this->credentials['username'])->Process($this->processId)->Tasks->createNew('name-test-1', 'event-test-1', true, false, 'message-test-1');
        $this->sdk->Profile($this->credentials['username'])->Process($this->processId)->Tasks->createNew('name-test-2', 'event-test-2', false, true, 'message-test-2');
        $response = $this->sdk->Profile($this->credentials['username'])->Process($this->processId)->Tasks->listAll();
    ...
    {
        $response = $this->sdk->Profile($this->credentials['username'])->Process($this->processId)->Tasks->createNew('name-test', 'event-test', true, false, 'message-test');
    ...
    {
        $response = $this->sdk->Profile($this->credentials['username'])->Process($this->processId)->Tasks->createNew('náme-tést', 'event-test', true, false, 'message-test');
